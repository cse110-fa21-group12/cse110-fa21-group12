...
saveRecipeButton.addEventListener("click", () => {
  const a = document.getElementById("title").value,
    ...
    k = [];
  for (let a = 0; a < h.length; a++) k[a] = [j[a].value, h[a].value];
  const l = document.getElementsByClassName("directions"),
    m = [];
  for (let a = 0; a < l.length; a++) m[a] = l[a].value;
  const n = JSON.stringify({
    ...
    o = document.getElementById("file-ip-1"),
    p = new FormData();
  p.append("json", n),
    p.append("img", o.value),
    fetch("/recipes/create", { method: "PUT", body: p })
      .then((a) => a.json())
      .then((a) => {

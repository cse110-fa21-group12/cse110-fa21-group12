...
saveRecipeButton.addEventListener("click", () => {
  const a = document.getElementById("title").value,
    ...
    k = [];
  for (let a = 0; a < c.length; a++) k[a] = c[a].value;
  const l = [];
  for (let a = 0; a < d.length; a++) l[a] = d[a].value;
  const m = new Map();
  for (let a = 0; a < h.length; a++) m.set(j[a].value, h[a].value);
  ...
  const p = JSON.stringify({
    ...
    r = new FormData();
  r.append("json", p),
    r.append("img", q.value),
    fetch("/recipes/create", { method: "PUT", body: r })
      .then((a) => a.json())
      .then((a) => {
